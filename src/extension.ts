...
class AntigravityViewProvider implements vscode.WebviewViewProvider {
    ...
    const PREDEFINED_PROMPTS = [
        { id: 'create .net console project', label: 'Create .NET console project', prompt: 'Create a .NET console-based menu-driven CRUD application. Requirements: - Domain: {DOMAIN_NAME} - Storage Type: {Static List / In-Memory DB} - Model Fields Count: {NUMBER_OF_FIELDS} - Test Cases Required: {NUMBER_OF_TESTCASES} Instructions: - Use only one model class and place it inside a folder named "Models". - Implement full CRUD operations. - Keep all logic inside Program.cs. - Use auto-increment Id. - Add basic input validation. - Application must be menu-driven and run until Exit is selected. - Generate meaningful fields based on the given domain. - Provide manual test cases as requested.', needsFolder: false },
        { id: 'create description' , label: 'Create project description', prompt: 'Wriet a scenario based project description for this project by understanding the complete solution and testcases, in the description, no need to have the tescases details but the tescases expects some string matches, methods, its parameter, return type, or the logics have be written in the exact format, those needs to be inculded while explaing the flow itself, since testcases may expects those things. it should not have seperate sertion for testcases', needsFolder: false },
